...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    // --- OPTIMIZATION 1: Cap Pixel Ratio ---
    // Instead of window.devicePixelRatio (which can be 3x on phones), we force 1.
    // This reduces GPU load by 300%+
  ...
  initPasses() {
    ...
    // --- OPTIMIZATION 2: Reduce Bloom Resolution ---
    // luminanceThreshold: 0.2
    // resolutionScale: 0.5 (Half resolution for glow saves FPS)
    this.bloomPass = new EffectPass(
      this.camera,
      new BloomEffect({
        luminanceThreshold: 0.2,
        luminanceSmoothing: 0,
        resolutionScale: 0.5 // Lowered from 1 to 0.5
      })
    );

    // --- OPTIMIZATION 3: REMOVED SMAA (Anti-Aliasing) ---
    // SMAA is extremely heavy. For a fast background, we don't need it.
    // It has been removed from this function.

    this.renderPass.renderToScreen = false;
    this.bloomPass.renderToScreen = true; // Set Bloom to render to screen since SMAA is gone
  ...
  loadAssets(): Promise<void> {
    ...
    return new Promise(resolve => {
      resolve(); // Resolved immediately as we removed SMAA assets loading
    });
  }
